...
GPU_ID="${1:-0}"
MODE="${2:-full}"
BASE_CFG="${3:-config/base_hpo_stage1_reconst_28.yaml}"

PRETRAIN_CFG="config/pretrain_reconst.yaml"
...
if [[ "${MODE}" == "full" ]]; then
    bash scripts/pretrain_reconst.sh "${BASE_CFG}" "${PRETRAIN_CFG}"
...
echo "[DONE] run_hpo_stage1_reconst.sh finished."
