...
# Be polite on shared CPU servers. Each GPU job gets one DataLoader worker and
# numerical libraries are prevented from spawning large thread pools.
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-1}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-1}"
export OPENBLAS_NUM_THREADS="${OPENBLAS_NUM_THREADS:-1}"
export NUMEXPR_NUM_THREADS="${NUMEXPR_NUM_THREADS:-1}"

GPUS_CSV="${FILTER_RMS_GPUS:-1,2}"
...
TASKS=(
  "pohang|contrast|0.05,0.10,0.25,0.50,1.00|stage1_pohang_only|pohang_contrast"
  "pohang|reconst|1.00|stage1_pohang_only|pohang_reconst_reconst_noanom"
  "pohang|reconst_noanom|0.05,0.10,0.25,0.50,1.00|stage1_pohang_only|pohang_reconst_reconst_noanom"
...
launch_task() {
  ...
  (
    ...
    export SITE_STUDY_LOG_SLUG="${DATASET}_${log_slug}_remaining_gpu12"
...
while true; do
  ...
  if [[ "${all_idle}" == "true" && "${task_index}" -ge "${#TASKS[@]}" ]]; then
    echo "[$(date '+%F %T')] [COMPLETE] filter_rms remaining tasks finished" | tee -a "${LOG_ROOT}/scheduler.log"
    break
  fi
  sleep 60
done
