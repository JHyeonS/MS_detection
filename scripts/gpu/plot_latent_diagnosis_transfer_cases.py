...
OUT_DIR = ROOT / "figures" / "current_results_summary" / "latent_diagnosis"
GAIN_CSV = ROOT / "figures" / "current_results_summary" / "cross_site_summary" / "transfer_gain_decomposition_metrics.csv"
...
PREPROC_ORDER = ["filter_rms", "logenv"]
...
def balanced_umap_sample(df: pd.DataFrame, max_per_group: int = 180, seed: int = 12) -> pd.DataFrame:
    sampled = []
...
def collect_preproc(preproc: str, spec: dict) -> tuple[pd.DataFrame, dict, dict]:
    ...
    pred = pd.read_csv(run_dir / "test" / EXPERIMENT / "test_predictions.csv")
...
def load_gain_summary() -> pd.DataFrame:
    gain = pd.read_csv(GAIN_CSV)
    sub = gain[
        gain["source_site"].eq(SOURCE_SITE)
        & gain["target_site"].eq(TARGET_SITE)
        & np.isclose(gain["fraction"], FRACTION)
        & gain["preprocessing"].isin(PREPROC_ORDER)
    ].copy()
    return sub
...
def draw_umap(ax: plt.Axes, emb: pd.DataFrame, title: str, metrics: dict, letter: str) -> None:
    ...
    alphas = {"Source": 0.42, "Target": 0.78}
...
def draw_gain(ax: plt.Axes, gain_df: pd.DataFrame) -> None:
    ...
    for idx, gain in enumerate(gains):
        ax.text(idx, max(scratch[idx], cross[idx]) + 0.035, f"gain {gain:+.2f}", ha="center", va="bottom", fontsize=8.0)
    ax.set_ylim(0, 1.08)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=16, ha="right")
    ax.set_ylabel("Balanced accuracy", fontsize=8.5)
    ax.set_title("Case selection by transfer gain", fontsize=10.5, fontweight="normal", pad=8)
    ax.text(
        0.02,
        0.98,
        "A",
    ...
    style_panel(ax)
    ax.legend(frameon=False, fontsize=7.8, loc="upper center", bbox_to_anchor=(0.5, -0.12), ncol=1)
...
def draw_swd(ax: plt.Axes, swd_rows: pd.DataFrame) -> None:
    ...
    width = 0.34
    for offset, preproc in zip([-width / 2, width / 2], PREPROC_ORDER):
    ...
    ax.set_title("Latent distribution mismatch", fontsize=10.5, fontweight="normal", pad=8)
    ax.text(
        0.02,
        0.98,
        "D",
...
def main() -> None:
    ...
    emb_df.to_csv(OUT_DIR / "utah2019_to_utah2023_frac0p25_source_target_umap_points.csv", index=False)
    swd_df.to_csv(OUT_DIR / "utah2019_to_utah2023_frac0p25_source_target_swd.csv", index=False)
    metric_df.to_csv(OUT_DIR / "utah2019_to_utah2023_frac0p25_transfer_metrics.csv", index=False)

    fig = plt.figure(figsize=(13.5, 4.15))
    gs = fig.add_gridspec(1, 4, width_ratios=[1.02, 1.35, 1.35, 1.02], wspace=0.36)
    draw_gain(fig.add_subplot(gs[0, 0]), gain)
    for idx, preproc in enumerate(PREPROC_ORDER):
        ...
        draw_umap(ax, emb, f"{PREPROCESSING[preproc]['label']} latent UMAP", metrics, "B" if idx == 0 else "C")
    draw_swd(fig.add_subplot(gs[0, 3]), swd_df)
    ...
    fig.suptitle(
        f"Latent diagnosis of transfer failure and recovery ({SITE_LABELS[SOURCE_SITE]} -> {SITE_LABELS[TARGET_SITE]}, fraction={FRACTION_LABEL})",
        fontsize=12.2,
        y=0.985,
    )
    fig.subplots_adjust(left=0.055, right=0.99, bottom=0.22, top=0.78, wspace=0.36)
    save(fig, "latent_diagnosis_failure_recovery_utah2019_to_utah2023_frac0p25")

    print(f"[DONE] wrote latent diagnosis figure to {OUT_DIR}")
