...
def load_result(json_path: Path, source: str, branch: str) -> Optional[Dict[str, Any]]:
    js = _safe_read_json(json_path)
    if js is None:
        return None
    exp_name = json_path.parent.name if source == "test" else json_path.parent.parent.name
    row = {"experiment": exp_name, "metrics_path": str(json_path), "source": source}
...
def collect_results(run_root: Path, source: str, branch: str) -> pd.DataFrame:
    if source == "test":
    ...
    if not paths:
        raise FileNotFoundError(f"No result files found under run_root={run_root}, source={source}")

    rows = []
    for p in paths:
        row = load_result(p, source=source, branch=branch)
        if row is not None:
            rows.append(row)
    if not rows:
        raise RuntimeError(f"Found result files, but none matched branch='{branch}' for source='{source}'.")
...
def print_topk(df: pd.DataFrame, topk: int, source: str, branch: str, metric: str):
    cols = [
        "rank", "experiment", "method", "num_layers", "latent_dim",
...
def build_default_out_csv(run_root: Path, source: str, branch: str, topk: int) -> Path:
    return run_root / f"top{topk}_{source}_{branch}_ranking.csv"
...
def main():
    ...
    df = collect_results(run_root, args.source, args.branch)
    ...
    out_csv = Path(args.out_csv) if args.out_csv else build_default_out_csv(run_root, args.source, args.branch, args.topk)
