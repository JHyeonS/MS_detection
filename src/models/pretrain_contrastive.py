...
def _get_attr(obj, name, default=None):
    """
    OmegaConf / argparse namespace / dict 호환용 안전 getter
    """
    if hasattr(obj, name):
...
class CAEDecoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.dropout = float(_get_attr(enc_cfg, "dropout", 0.0))
        ...
        # encoder output channel progression before proj:
        # [base, 2*base, 4*base, ...]
        enc_channels = [self.base_channels * (2 ** i) for i in range(self.num_layers)]

        # decoder는 latent_dim에서 시작해서 reversed enc_channels로 복원
        ...
        for i, out_ch in enumerate(rev_channels):
            # encoder reversed:
            # last encoder blocks were stride=(2,2)
            # first encoder block was stride=(1,2)
            #
            # decoder는 대부분 (2,2) upsample,
            # 마지막 block만 (1,2) upsample
    ...
    def forward(self, feat: torch.Tensor, output_size=None) -> torch.Tensor:
        ...
        # transpose conv의 rounding 차이를 마지막에 crop해서 맞춤
...
def build_recon_loss(cfg):
    ...
    loss_name = _get_attr(pre_cfg, "recon_loss", "l1").lower()
