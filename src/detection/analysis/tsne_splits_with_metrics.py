...
- Silhouette / DBI는 t-SNE 2D가 아니라 standardize + optional PCA feature에서 계산한다.
"""
...
def plot_split_style(df: pd.DataFrame, title: str, out_png: Path, figsize=(11, 9), dpi=220):
    ...
    plt.xlabel("t-SNE 1")
    plt.ylabel("t-SNE 2")
    plt.legend(markerscale=1.2, fontsize=15, ncol=2)
    plt.tight_layout()
...
def plot_site_style(df: pd.DataFrame, title: str, out_png: Path, figsize=(11, 9), dpi=220):
    ...
    plt.legend(markerscale=1.2, fontsize=12)
    plt.tight_layout()
    plt.savefig(out_png, dpi=dpi)
    plt.close()
...
def prepare_metric_features(
    ...
    if use_pca:
        ...
        if n_samples >= 2:
            use_dim = max(2, min(pca_dim, n_dim, n_samples - 1))
            x = PCA(n_components=use_dim, random_state=seed).fit_transform(x)
...
def compute_cluster_metrics(
    ...
    if df.empty:
        result["status"] = "empty_df"
        return result

    if label_col not in df.columns:
        result["status"] = f"missing_label_col:{label_col}"
        return result

    labels = df[label_col].astype(str).to_numpy()
    uniq = np.unique(labels)
    result["n_clusters"] = int(len(uniq))

    if len(uniq) < 2:
        result["status"] = "need_at_least_2_clusters"
        return result

    vc = pd.Series(labels).value_counts()
    if (vc < 2).any():
        result["status"] = "some_clusters_have_less_than_2_samples"
        return result

    feats = np.stack(df["feat"].to_list(), axis=0)
    feats_for_metric = prepare_metric_features(
        feats,
        seed=seed,
        pca_dim=pca_dim,
        use_pca=use_pca,
    ...
    result["silhouette"] = float(silhouette_score(feats_for_metric, labels))
    result["davies_bouldin"] = float(davies_bouldin_score(feats_for_metric, labels))
    return result


def save_metrics_json(metrics: Dict[str, Any], out_path: Path):
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)
    print(f"[INFO] saved metrics: {out_path}")


def print_metric_block(name: str, metric: Dict[str, Any]):
    print("\n" + "=" * 80)
    print(f"[METRIC] {name}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="PCA dim used for Silhouette / DBI calculation",
    )
    parser.add_argument(
        "--disable_metric_pca",
        action="store_true",
        help="If set, compute Silhouette / DBI on standardized original features without PCA",
    )
    ...
    metric_use_pca = not args.disable_metric_pca
    ...
    if not raw_df.empty:
        ...
        raw_metrics = {
            "class_name": compute_cluster_metrics(
                raw_df,
                label_col="class_name",
                seed=args.seed,
                pca_dim=args.metric_pca_dim,
                use_pca=metric_use_pca,
            ),
            "split": compute_cluster_metrics(
                raw_df,
                label_col="split",
                seed=args.seed,
                pca_dim=args.metric_pca_dim,
        ...
        if has_known_domain(raw_df):
            known_raw_df = raw_df[raw_df["domain"].isin(["pohang", "utah"])].copy()
            raw_metrics["domain"] = compute_cluster_metrics(
                known_raw_df,
                label_col="domain",
                seed=args.seed,
                pca_dim=args.metric_pca_dim,
            ...
            raw_domain_class_df = known_raw_df.copy()
            raw_domain_class_df["domain_class"] = (
                raw_domain_class_df["domain"].astype(str) + "_" + raw_domain_class_df["class_name"].astype(str)
            )
            raw_metrics["domain_class"] = compute_cluster_metrics(
                raw_domain_class_df,
                label_col="domain_class",
                seed=args.seed,
                pca_dim=args.metric_pca_dim,
                use_pca=metric_use_pca,
            )

        metrics_summary["raw"] = raw_metrics
        print_metric_block("raw", raw_metrics)
        save_metrics_json(raw_metrics, out_dir / "raw_cluster_metrics.json")
    ...
    if args.ckpt is not None:
        ...
        if not enc_df.empty:
            ...
            if has_known_domain(enc_df):
                known_enc_df = enc_df[enc_df["domain"].isin(["pohang", "utah"])].copy()
                enc_metrics["domain"] = compute_cluster_metrics(
                    known_enc_df,
                    label_col="domain",
                    seed=args.seed,
                    pca_dim=args.metric_pca_dim,
                ...
                enc_domain_class_df = known_enc_df.copy()
                enc_domain_class_df["domain_class"] = (
                    enc_domain_class_df["domain"].astype(str) + "_" + enc_domain_class_df["class_name"].astype(str)
                )
                enc_metrics["domain_class"] = compute_cluster_metrics(
                    enc_domain_class_df,
                    label_col="domain_class",
                    seed=args.seed,
                    pca_dim=args.metric_pca_dim,
            ...
            metrics_summary["encoder"] = enc_metrics
            print_metric_block("encoder", enc_metrics)
            save_metrics_json(enc_metrics, out_dir / "encoder_cluster_metrics.json")
    else:
    ...
    summary = {
        ...
        "metric_pca_dim": int(args.metric_pca_dim),
        "metric_use_pca": bool(metric_use_pca),
        "requested_outputs": [
            "tsne_raw_split.png",
        ...
        "optional_outputs": [
            ...
            "raw_cluster_metrics.json",
            "encoder_cluster_metrics.json",
        ],
        "cluster_metrics": metrics_summary,
    }
